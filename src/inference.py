...
def load_prediction_artifacts():
    ...
    return model, label_encoder, feature_names, medians
...
def clean_input_features(df, feature_names):
    ...
    # Drop identifiers to match training
...
def predict_dataframe(df):
    model, label_encoder, feature_names, medians = load_prediction_artifacts()
    aligned_df = clean_input_features(df, feature_names)
    
    # Use saved training medians instead of calculating on the fly
    aligned_df = aligned_df.fillna(medians)
